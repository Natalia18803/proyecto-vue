<template>
  <div class="pokemon-card" :style="obtenerFondoGradiente()">
...
export default {
  ...
  methods: {
    obtenerFondoGradiente() {
      const tipos = this.pokemon.types;
      if (tipos.length === 1) {
        const color = this.coloresTipos[tipos[0].type.name];
        return {
          background: `linear-gradient(135deg, ${color} 0%, ${color}99 100%)`
        };
      } else if (tipos.length === 2) {
        const color1 = this.coloresTipos[tipos[0].type.name];
        const color2 = this.coloresTipos[tipos[1].type.name];
        return {
